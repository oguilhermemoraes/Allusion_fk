...
export interface IMenuSliderItem {
  ...
  /** When false, the slider is continuously adjustable: clicking the track does not snap to the datalist ticks. */
  snap?: boolean;
}
          ...
          <div className="slider-ticks">
            {options.map((o) => (
              <span
                key={`${o.value}-tick`}
                className="tick"
                style={{ left: `${((o.value - min) / (max - min)) * 100}%` }}
              >
                {o.label}
              </span>
            ))}
