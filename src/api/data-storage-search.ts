...
/** A condition on the dominant palette: value is a fixed color id (e.g. 'blue'). */
export type ColorConditionDTO = {
  key: 'palette';
  operator: ColorOperatorType;
  value: string;
  valueType: 'color';
};
...
export type ExtractKeyByValue<T, V> = {
  [K in keyof T]: [T[K]] extends [V | undefined] ? (K extends string ? K : never) : never;
}[keyof T];
...
export const ColorOperators = ['contains', 'notContains'] as const;
export type ColorOperatorType = (typeof ColorOperators)[number];
