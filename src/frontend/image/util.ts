...
/** Decoders that decode a file directly from its path (Tauri native commands). */
export interface PathDecoder {
  decodePath: (path: string) => Promise<ImageData>;
}

/**
 * Decodes an image using the native path decoder in the Tauri runtime,
 * or by reading the file buffer otherwise.
 */
export async function decodeImage(
  decoder: Decoder | PathDecoder,
  path: string,
): Promise<ImageData> {
  if ('decodePath' in decoder) {
    return decoder.decodePath(path);
  }
...
export async function getBlob(decoder: Decoder | PathDecoder, path: string): Promise<string> {
