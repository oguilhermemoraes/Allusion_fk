...
import { FIXED_COLORS_BY_ID } from '../../../common/color';
...
export abstract class ClientFileSearchCriteria implements IBaseSearchCriteria {
  ...
  @observable public valueType: 'number' | 'date' | 'string' | 'array' | 'color';
  ...
  constructor(
    key: keyof FileDTO,
    valueType: 'number' | 'date' | 'string' | 'array' | 'color',
  ...
  static deserialize(criteria: SearchCriteria): ClientFileSearchCriteria {
    ...
    switch (valueType) {
      ...
      case 'color':
        const color = criteria as IColorSearchCriteria;
        return new ClientPaletteColorCriteria(color.key, color.value, color.operator);
      default:
...
export class ClientPaletteColorCriteria extends ClientFileSearchCriteria {
  @observable public value?: string;

  constructor(key: keyof FileDTO, value?: string, operator: ColorOperatorType = 'contains') {
    super(key, 'color', operator);
    this.value = value;
    makeObservable(this);
  }

  @action.bound getLabel: (dict: SearchKeyDict) => string = (dict) => {
    const colorName = this.value ? FIXED_COLORS_BY_ID[this.value].name : undefined;
    return `${dict[this.key] || camelCaseToSpaced(this.key as string)} ${
      this.operator === 'contains' ? 'contains' : 'does not contain'
    } ${colorName ?? this.value ?? 'any color'}`;
  };

  serialize = (): IColorSearchCriteria => {
    return {
      key: this.key,
      valueType: this.valueType,
      operator: this.operator as ColorOperatorType,
      value: this.value ?? '',
    };
  };

  toCondition = (): ColorConditionDTO => {
    return this.serialize() as ColorConditionDTO;
  };

  @action.bound setOperator(op: ColorOperatorType): void {
    this.operator = op;
  }

  @action.bound setValue(value: string): void {
    this.value = value;
  }
}
