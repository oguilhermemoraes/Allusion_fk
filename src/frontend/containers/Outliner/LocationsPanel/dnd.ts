...
export const isAcceptableType = (e: React.DragEvent) => {
  const ok = e.dataTransfer.types.some(
...
/**
 * DropEffect the target should advertise. It must be inside the source's
 * `effectAllowed` (move for internal drags, copy for external OS drops),
 * otherwise the browser cancels the drop (dropEffect silently becomes 'none').
 */
export const getDropEffect = (e: React.DragEvent) =>
  e.dataTransfer.types.includes(DnDFileType) ? 'move' : 'copy';
...
export const findDroppedFileMatches = action(
  (dropData: (File | string)[], fs: FileStore): ClientFile[] | false => {
    ...
    return matches.every((m): m is ClientFile => !!m) ? matches : false;
  },
);
...
export function onDragOver(event: React.DragEvent<HTMLDivElement>): boolean {
  ...
  if (isFile) {
    // The dropEffect must be within the source's effectAllowed, otherwise the
    // browser cancels the drop (dropEffect becomes 'none' and `drop` never
    // fires). Internal drags declare `effectAllowed='move'`; external OS drops
    // typically allow copy, so keep 'copy' for them.
...
export async function storeDroppedImage(dropData: (string | File)[], directory: string) {
  for (const dataItem of dropData) {
    ...
    if (dataItem instanceof File) {
      const filePath = dataItem.path;
      const buffer = filePath ? await fse.readFile(filePath) : dataItem;
      fileData = {
        directory,
        filenameWithExt: filePath ? path.basename(filePath) : dataItem.name,
