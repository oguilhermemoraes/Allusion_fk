...
interface ITreeData {
  ...
  createSubfolder: (parentPath: string) => void;
...
const customKeys = (
  ...
  switch (event.key) {
    ...
    case 'F2':
      event.stopPropagation();
      treeData.renameFolder(nodeData.path, nodeData.name);
      break;
...
const DirectoryMenu = observer(
  ({
    ...
    const handleOpenFileExplorer = useCallback(() => showItemInFolder(path), [path]);
      ...
      show(
        ...
        <Menu>
          <DirectoryMenu
            ...
            onCreateSubfolder={treeData.createSubfolder}
            onRenameFolder={treeData.renameFolder}
          />
        </Menu>,
      ),
    [nodeData, show, treeData.exclude, treeData.createSubfolder, treeData.renameFolder],
  );
...
const Location = observer(
  ({ nodeData, treeData }: { nodeData: ClientLocation; treeData: ITreeData }) => {
    ...
    const handleContextMenu = useCallback(
      ...
      [show, nodeData, onDelete, treeData.exclude, treeData.createSubfolder, treeData.renameFolder],
    );
    ...
    const handleDrop = useCallback(
      ...
      [dndData.source, fileDnD, locationStore, nodeData],
    );
    ...
    return (
      ...
      >
        ...
        {nodeData.isRefreshing && (
          <span className="location-refreshing-icon">{IconSet.RELOAD_COMPACT}</span>
        )}
      </div>
    );
  },
...
const LocationsTree = ({
  ...
  const treeData: ITreeData = useMemo<ITreeData>(
    ...
    [expansion, onDelete, onExclude, onCreateSubfolder, onRenameFolder],
  );
...
const LocationsPanel = observer((props: Partial<MultiSplitPaneProps>) => {
  ...
  const [folderToCreateIn, setFolderToCreateIn] = useState<string>();
  const [folderToRename, setFolderToRename] = useState<{ path: string; name: string }>();
  ...
  return (
    ...
    >
      <LocationsTree
        ...
        onCreateSubfolder={setFolderToCreateIn}
        onRenameFolder={(path, name) => setFolderToRename({ path, name })}
      />
      ...
      {folderToCreateIn && (
        <CreateFolderDialog
          open={Boolean(folderToCreateIn)}
          parentPath={folderToCreateIn}
          onClose={() => setFolderToCreateIn(undefined)}
        />
      )}
      {folderToRename && (
        <RenameFolderDialog
          open={Boolean(folderToRename)}
          currentPath={folderToRename.path}
          currentName={folderToRename.name}
          onClose={() => setFolderToRename(undefined)}
        />
      )}
    </MultiSplitPane>
  );
