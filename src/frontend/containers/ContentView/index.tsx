...
const Content = observer(() => {
  ...
  const { marquee, handleMouseDown, isBoxSelecting } = useBoxSelect(container);
  ...
  const clearFileSelection = useAction((e: React.MouseEvent | React.KeyboardEvent) => {
    ...
    if (!uiStore.isSlideMode && isLayout && !isBoxSelecting.current) {
  ...
  return (
    <div
      ...
      style={
        {
          '--grid-border-radius': `${uiStore.gridBorderRadius}px`,
          '--grid-border-width': `${uiStore.gridBorderWidth}px`,
    ...
    >
      ...
      <ManyOpenExternal />
      <RenameFileDialog
        open={Boolean(uiStore.fileToRename)}
        file={uiStore.fileToRename}
        onClose={uiStore.closeRenameFileDialog}
      />
    </div>
  );
});
