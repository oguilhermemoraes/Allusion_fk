...
export type Criteria =
  ...
  | Field<'palette', ColorOperatorType, ColorValue>
...
export type Value = string | number | Date | TagValue | ColorValue;
...
export function fromCriteria(criteria: ClientFileSearchCriteria): [ID, Criteria] {
  ...
  if (
    ...
    query.value = id;
  } else if (criteria instanceof ClientPaletteColorCriteria && criteria.key === 'palette') {
    query.value = criteria.value;
  } else if (
...
export function intoCriteria(query: Criteria, tagStore: TagStore): ClientFileSearchCriteria {
  ...
  } else if (query.key === 'palette') {
    return new ClientPaletteColorCriteria('palette', query.value, query.operator);
  } else {
