...
import { FIXED_COLORS, FIXED_COLORS_BY_ID } from 'common/color';
import { IconSet } from 'widgets';
import { ColorOperators, NumberOperators, StringOperators } from '../../../api/data-storage-search';
...
import { Criteria, ColorValue, Key, Operator, TagValue, Value, defaultQuery } from './data';
...
export const KeySelector = forwardRef(function KeySelector(
  ...
  const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    dispatch((criteria) => {
      // Keep the text value and operator when switching between name, path and description
      if ([criteria.key, key].every((k) => ['name', 'absolutePath', 'description'].includes(k))) {
        criteria.key = key;
        return { ...criteria };
      } else {
...
export const ValueInput = ({ labelledby, keyValue, value, dispatch }: FieldInput<Value>) => {
  ...
  } else if (keyValue === 'palette') {
    return <ColorInput labelledby={labelledby} value={value as ColorValue} dispatch={dispatch} />;
...
const ColorInput = ({ value, dispatch }: ValueInput<ColorValue>) => {
  const selected = value !== undefined && value !== '' ? FIXED_COLORS_BY_ID[value] : undefined;
  const handleSelect = (colorId: string) => {
    dispatch(setValue(colorId));
  };
  ...
  return (
    <span className="palette-color-input">
      ...
      {selected !== undefined && (
        <button type="button" className="btn-icon btn-icon-large" onClick={handleDeselect}>
          {IconSet.CLOSE}
          <span className="visually-hidden">Clear color</span>
        </button>
      )}
    </span>
  );
...
function getOperatorOptions(key: Key) {
  if (['dateAdded', 'size', 'width', 'height'].includes(key)) {
    ...
    return ColorOperators.map((op) => toOperatorOption(op));
  }
  return [];
}
