...
class RootStore {
  ...
  static async main(backend: DataStorage, backup: DataBackup): Promise<RootStore> {
    const rootStore = new RootStore(backend, backup, (fileStore, uiStore) => {
      ...
      if (uiStore.isSlideMode && fileStore.fileList.length > 0 && activeFile) {
    ...
    fileStoreInit()
      ...
      .catch((err) => {
        // The window is created hidden (`visible: false`): even when loading the
        // initial files fails we must still reveal it, otherwise the app hangs as
        // an invisible window with no feedback to the user.
        console.error('Failed to load the initial files', err);
      })
      .finally(() => {
        // Window is created hidden (Tauri `visible: false`): only show it now that the
        // initial files are loaded, so the user never sees an empty gallery flash.
    ...
    rootStore.locationStore.watchLocations().then((foundNewFiles) => {
      ...
      // One-time backfill of the dominant palette for images indexed before #66.
      rootStore.fileStore.backfillPalettes().catch(console.error);
    });
