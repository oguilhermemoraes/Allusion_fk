...
import { registerAssetScope } from '../services/assetScope';
...
class LocationStore {
  ...
  @action async init(): Promise<void> {
    ...
    // Tauri: expose the location roots through the asset:// protocol (scope)
    registerAssetScope(locations.map((l) => l.path));
  }
  ...
  @action.bound async create(path: string): Promise<ClientLocation> {
    ...
    registerAssetScope([path]);
    return location;
  }

  @action.bound async createSubFolder(parentPath: string, folderName: string): Promise<string> {
    const newPath = await RendererMessenger.createFolder(parentPath, folderName);
    const location = this.locationList.find(
      (l) => parentPath.startsWith(l.path) || parentPath === l.path,
    );
    if (location) {
      await location.refreshSublocations();
    }
    return newPath;
  }
  ...
  @action.bound async renameFolder(oldPath: string, newName: string): Promise<string> {
    ...
    if (oldPath === location.path) {
      const parent = SysPath.dirname(oldPath);
      const newPath = SysPath.join(parent, newName);
      await RendererMessenger.renamePath(oldPath, newName);
      await this.changeLocationPath(location, newPath);
      return newPath;
    }

    const newPath = await RendererMessenger.renamePath(oldPath, newName);

    const normOld = SysPath.normalize(oldPath);
    const normNew = SysPath.normalize(newPath);
    const oldPrefix = normOld.endsWith(SysPath.sep) ? normOld : `${normOld}${SysPath.sep}`;
    const newPrefix = normNew.endsWith(SysPath.sep) ? normNew : `${normNew}${SysPath.sep}`;
    const locFiles = await this.backend.fetchFilesByKey('locationId', location.id);
    const affectedFiles = locFiles.filter((f) =>
      SysPath.normalize(f.absolutePath).startsWith(oldPrefix),
    );

    if (affectedFiles.length > 0) {
      const normLocPath = SysPath.normalize(location.path);
      const updatedFiles: FileDTO[] = affectedFiles.map((f) => {
        const normAbs = SysPath.normalize(f.absolutePath);
        const newAbs = normAbs.replace(oldPrefix, newPrefix);
        return {
          ...f,
          absolutePath: newAbs,
          relativePath: newAbs.replace(normLocPath, ''),
        };
      });
      await this.backend.saveFiles(updatedFiles);
      this.rootStore.fileStore.refetch();
