...
import { paletteContainsColor } from '../../common/color';
...
function filterWhere<T>(
  ...
  switch (crit.valueType) {
    ...
    case 'color':
      return filterColorLambda(crit);
  }

  const _exhaustiveCheck: never = crit;
...
function filterColorLambda<T>(crit: ColorConditionDTO): (val: T) => boolean {
  const { value } = crit;
  return (val: T): boolean => {
    const palette = (val as any).palette as FileDTO['palette'];
    if (crit.operator === 'notContains') {
      return !paletteContainsColor(palette, value);
    }
    return paletteContainsColor(palette, value);
  };
}
